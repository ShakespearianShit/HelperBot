...
var realTime;
var apiKey = "17234476-aeccb2b84ce9c11e60f27162d";

client.on("ready", () => {
  console.log("yes");
  client.user.setActivity("Willy's bullshit", { type: "LISTENING" });
});
...
if (command === "leave") {
  ...
  if (command === "suggest") {
	const guild = client.guilds.cache.get("720145093390106626")
...
if (command === "cat") {
var r = await pixabay.searchImages(apiKey, 'cat')
...
if (command === "foxy") {
var r = await pixabay.searchImages(apiKey, 'fox')
message.channel.send(new Discord.MessageEmbed()
     .setTitle("Lookin kinda foxy :flushed:")
     .setImage(r.hits[Math.floor(Math.random() * r.hits.length)].largeImageURL))
}

  if (command === "pupper" || command === "doge") {
const request = require('request');
...
request.get('https://dog.ceo/api/breeds/image/random', {
  ...
  if (command === "colorscheme") {
    ...
    const embed = new Discord.MessageEmbed()
	.setAuthor("Willy's asshole bot :sunglasses:")
	.setColor(firstColor.replace("#", "") ? firstColor.replace("#", "") : firstColor)
	.addField("Colors (hex)", scheme.colors().slice(0, 10).toString())
	.addField("Preview link", `https://coolors.co/${scheme.colors().slice(0, 10).toString().replace(/,/g, "-")}`)
    message.channel.send(embed);
}
